...
(defn ^:export ^:export lat-lon
  ...
  ([m]
   (let [lat (:lat m)
     ...
     (lat-lon lat lon)))
  ([lat lon]
  (js/google.maps.LatLng. lat lon)))

(defn ^:export make-handler [obj fnc]
  (fn [ & args]
    (apply fnc obj args)))

(defn ^:export random-point
  ([] (random-point rand))
  ([rand]
   (lat-lon (rand-in-range -85 85)
            (rand-in-range -170 170))))

(defn ^:export make-component-fnc 
  ([klass option-keys]
